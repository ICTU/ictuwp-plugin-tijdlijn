...
if ( ! class_exists( 'RHSWP_timelineplugin' ) ) :
    ...
    class RHSWP_timelineplugin {
        ...
        private function includes() {
            ...
            require_once dirname( __FILE__ ) . '/tijdlijncpt.gutenberg-block.php';
        ...
        public function register_shortcode( $atts ) {
            ...
            if ( ! $rhswptimeline || $rhswptimeline->post_status != 'publish' || $rhswptimeline->post_type != RHSWP_CPT_TIMELINE ) {
                ...
                if ( $preview ) {
                    ...
                    if ( $title) {
                        return '<p class="error">' . sprintf( _x( 'The timeline with ID "%s" and title "%s" is not (yet) published.', '404 error', "rhswp-timeline" ), $id, $title ) . '</p>';
                    }else {
                        return '<p class="error">' . sprintf( _x( 'No timeline found with ID "%s".', '404 error', "rhswp-timeline" ), $id ) . '</p>';
                    }
                } else {
        ...
        public function register_frontend_style_script() {
            if ( ! is_admin() ) {
                ...
                wp_enqueue_style( 'ictuwp-plugin-tijdlijn-frontend', RHSWP_TIMELINE_BASE_URL . 'assets/css/ictuwp-plugin-tijdlijn-frontend.css', array(), RHSWP_TIMELINE_VERSION, 'screen' );

                $version = RHSWP_TIMELINE_VERSION;
                $file    = 'timeline-min.js';

                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    // use only recent version
                    $file    = 'timeline.js';
                    $version = filemtime( dirname( __FILE__ ) . '/assets/js/' . $file );

                } else {
                    // use default version number
                }

                wp_enqueue_script( RHSWP_TIMELINE_JS_HANDLE, RHSWP_TIMELINE_ASSETS_URL . 'js/' . $file, array( 'jquery' ), $version, $infooter );
        ...
        public function rhswp_timeline_do_output( $timelineid, $dopreview = false ) {
            ...
            $returnstring = '<div class="timeline-container">';
            ...
            $returnstring .= '</div>'; // timeline-main
            $returnstring .= '</div>'; // timeline-container
        ...
        public function getuniqueid( $timeline_id ) {
            ...
            if ( isset( $post->ID ) ) {
                ...
                return 'timeline-' . $timeline_id . '-random-' . rand( 999, 3000 );
            }
        ...
        public function update_timeline() {
            ...
            if ( isset( $_POST['settings'] ) ) {
                ...
                $checkboxes = apply_filters( "RHSWP_TIMELINE_checkbox_settings", array(
                        'noConflict',
                        'fullWidth',
                        'hoverPause',
                        'links',
        ...
        public function admin_insert_rhwwptimeline_button( $context ) {
            ...
            foreach ( $available_post_types as $available_post_type ) {
                if ( defined( 'RHSWP_CPT_RIJKStimeline' ) && $available_post_type === RHSWP_CPT_RIJKStimeline ) {
                    continue;
                }
